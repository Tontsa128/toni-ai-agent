...
test("ComputerActionWorkflow re-observes inconclusive verification within policy bound", async () => {
  ...
  const lifecycle = {
    ...
    recordPostCondition(_s: string, _a: string, obs: ReturnType<typeof observation>, _e: unknown) {
  ...
  const observationsProvider = {
    async capture() {
      const next = observations[captures++];
      if (!next) throw new Error("Test observation provider exhausted.");
      return next;
    }
  };
  const workflow = new ComputerActionWorkflow(controller as never, lifecycle as never, observationsProvider, policy as never);
...
test("ComputerActionWorkflow stops after bounded inconclusive observations", async () => {
  ...
  const observationsProvider = { async capture() { captures += 1; return observation(`2026-09-17T12:00:0${captures}.000Z`); } };
  const workflow = new ComputerActionWorkflow(controller as never, lifecycle as never, observationsProvider, { decide: (_v: unknown, attempt: number) => attempt < 2 ? "observe_again" : "stop" } as never);
...
test("ComputerActionWorkflow rejects approval from another session", async () => {
  const controller = { async proposeAndQueue() { return { proposal: { action: { type: "click", x: 1, y: 2 }, observationCapturedAt: "2026-09-17T12:00:00.000Z", actionId: "a3" }, result: { ok: true, approved: false, output: { actionId: "a3" } } }; }, async approve() { throw new Error("must not execute"); }, reject() {} };
  const lifecycle = { recordProposal() {}, recordApprovalRequested() {}, recordApproval() {}, recordRejection() {}, recordExecution() { return { status: "accepted", reason: "ok" }; }, recordPostCondition() { return { status: "confirmed", reason: "ok" }; } };
  const workflow = new ComputerActionWorkflow(controller as never, lifecycle as never, { async capture() { return observation("2026-09-17T12:00:01.000Z"); } });

  await workflow.propose(observation("2026-09-17T11:59:59.000Z"), { type: "click", x: 1, y: 2 }, "owner-session");
  await assert.rejects(() => workflow.approveAndVerify("a3", "attacker-session"), /another session/);
});
