...
export class ApprovalRepository {
  ...
  public getActive(approvalId: string, now = Date.now()): ApprovalRecord | undefined {
    const row = this.db.prepare("SELECT * FROM approvals WHERE approval_id=? AND used=0 AND expires_at>?").get(approvalId, now) as ApprovalRow | undefined;
    return row ? mapRow(row) : undefined;
  }
  ...
  public consumeIfMatches(input: {
    ...
    return this.consumeIfMatchesAndReturn(input, now) !== undefined;
  }

  public consumeIfMatchesAndReturn(input: {
    approvalId:string; userId:string; sessionId:string; actionId:string; toolName:string; argumentHash:string;
  }, now = Date.now()): ApprovalRecord | undefined {
    const tx = this.db.transaction(() => {
      const row = this.db.prepare(`SELECT * FROM approvals
        ...
        AND argument_hash=? AND used=0 AND expires_at>?`).get(
          input.approvalId,input.userId,input.sessionId,input.actionId,input.toolName,input.argumentHash,now
        ) as ApprovalRow | undefined;
      if (!row) return undefined;
      const result = this.db.prepare("UPDATE approvals SET used=1 WHERE approval_id=? AND used=0").run(input.approvalId);
      return result.changes === 1 ? mapRow({ ...row, used: 1 }) : undefined;
    });
    return tx();
  }
...
function mapRow(row: ApprovalRow): ApprovalRecord {
  return {
    approvalId:row.approval_id,userId:row.user_id,sessionId:row.session_id,actionId:row.action_id,
