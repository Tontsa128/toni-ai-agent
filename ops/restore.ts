...
export async function restoreBackup(backupDirectory: string): Promise<void> {
  ...
  const backupDatabase = join(source, "toni.sqlite");
  await access(backupDatabase, constants.R_OK);
  await mkdir(join(paths.dataRoot, "database"), { recursive: true });
  await copyFile(backupDatabase, paths.databaseFile);
}
...
if (process.argv[1]?.endsWith("restore.js")) {
  if (!backupDirectory) {
    ...
    process.exitCode = 2;
  } else restoreBackup(backupDirectory).then(() => console.log("Backup restored. Start the application and run readiness checks.")).catch((error: unknown) => {
